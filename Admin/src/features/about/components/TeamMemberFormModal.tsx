...
interface Props {
  ...
  isLoading: boolean;
  onClose: () => void;
  onSubmit: (payload: { user_id: string; sort_order: number; is_active: boolean }) => void;
}

export default function TeamMemberFormModal({ isOpen, member, isLoading, onClose, onSubmit }: Props) {
  const {
    search,
    setSearch,
    page,
  ...
  } = useEligibleUsers(member?.user?.name ?? '');

  const [selectedUserId, setSelectedUserId] = useState<string>(member?.user_id || '');
  ...
  const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    onSubmit({ user_id: selectedUserId, sort_order: sortOrder, is_active: isActive });
  };
        ...
        <div className="modal-content">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="modal-footer">
              ...
              <button type="submit" className="btn btn-primary" disabled={isLoading || !selectedUserId}>
